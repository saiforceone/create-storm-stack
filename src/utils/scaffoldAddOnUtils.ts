 ...
 * @param projectPath
 */
async function installSentryForBackend(
  currentUrl: string,
  isWindows: boolean,
  projectPath: string
): Promise<ScaffoldOutput> {
  ...
  try {
    ...
    if (isWindows) webCoreAddonsPath = normalizeWinFilePath(webCoreAddonsPath);

    // read file
    const fileDataString = await readFile(webCoreAddonsPath, {
      encoding: 'utf-8',
    });
    const webCoreAddonsData = JSON.parse(
      fileDataString
    ) as STORMBackendAddonsFile;
    const sentryPkg = webCoreAddonsData.sentry.packages;
    const installString = `${
      COMMAND_CONSTANTS.CMD_PIPENV_INSTALL
    } ${Object.keys(sentryPkg)
      .map((s) => `${s}`)
      .join(' ')}`;
    ...
    if (isWindows)
      stormAddonsAutoloadPath = normalizeWinFilePath(stormAddonsAutoloadPath);

    await appendFile(
      stormAddonsAutoloadPath,
 ...
 */
export async function installSentryForFrontend(
  projectPath: string,
  ...
  try {
    ...
    if (isWindows)
      frontendAddonsFilePath = normalizeWinFilePath(frontendAddonsFilePath);
    const frontendAddonsData = await readFile(frontendAddonsFilePath, {
      encoding: 'utf-8',
    });
    const frontendAddonsPkg = JSON.parse(
      frontendAddonsData
    ) as STORMSpecificFrontendAddonsFile;
    ...
    const pkgString = Object.keys(fePackages)
      .map((p) => `${p}@${fePackages[p]}`)
      .join(' ');
...
export async function installSentry(
  ...
  try {
    ...
    const installBEResult = await installSentryForBackend(
      currentUrl,
      isWindows,
      projectPath
    );
